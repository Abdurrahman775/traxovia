...
export default function News() {
  ...
  const { data, isLoading, isError, isFetching, refetch, dataUpdatedAt } = useQuery({
    ...
    queryFn: () => {
      ...
      return api.get('/news', { params }).then(r => r.data as NewsResponse)
    },
      ...
      <div className="bg-s1 border border-s3 rounded-xl overflow-hidden">
        ...
        {isError && (
          <div className="px-5 py-10 text-center space-y-2">
            ...
            <div className="text-tx3 text-xs font-mono">Check that the Finnhub API key is configured in Admin → Settings.</div>
          </div>
        )}
